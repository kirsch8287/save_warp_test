...
def zcross_data_save(
        part=None, zPos=None, dirName="zposition_particle_data", 
            ...
            for i in range(nZPos):
                zWriteFile = open("./{dirName}/{fileName}_{zPos}m.txt".\
                                  format(dirName=dirName, fileName=fileName, 
                                         zPos=zPos[i]), "w")
                zHeaders = \
                "x(m){de}y(m){de}t(s){de}vx(m/s){de}vy(m/s){de}vz(m/s)".\
                    format(de=delim)
                ...
                for j in range(znn):
                    zWriteFile.write("\n")
                    zWriteFile.write("{xpos}{de}".format(
                            xpos=zxx[j], de=delim))
                    zWriteFile.write("{ypos}{de}".format(
                            ypos=zyy[j], de=delim))
    ...
    def savetsdata():
        for i in range(nTs):
            if top.it == ts[i]:
                ...
                for j in range(tsn):
                    tsWriteFile.write("\n")
                    tsWriteFile.write("{xpos}{de}".format(
                            xpos=tsx[j], de=delim))
                    tsWriteFile.write("{ypos}{de}".format(
                            ypos=tsy[j], de=delim))
                    tsWriteFile.write("{zpos}{de}".format(
                            zpos=tsz[j], de=delim))
...
def vtk_data_save(part=None, tsstart=None, tsend=None, tsint=1, 
